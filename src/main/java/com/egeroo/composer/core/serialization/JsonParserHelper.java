package com.egeroo.composer.core.serialization;
...
import com.egeroo.composer.core.error.ApiParameterError;
import com.egeroo.composer.core.error.PlatformApiDataValidationException;
...
public class JsonParserHelper {
    ...
    public BigDecimal extractBigDecimalName(final String parameterName, final JsonObject element, final Locale locale,
            final Set<String> modifiedParameters) {
        BigDecimal value = null;
        if (element.isJsonObject()) {
            final JsonObject object = element.getAsJsonObject();
            if (object.has(parameterName) && object.get(parameterName).isJsonPrimitive()) {
                ...
                final String valueAsString = primitive.getAsString();
                if (StringUtils.isNotBlank(valueAsString)) {
                    value = convertFrom(valueAsString, parameterName, Locale.forLanguageTag("en"));
                }
            }
        }
        return value;
    }
    public BigDecimal extractBigDecimalWithLocaleName(final String parameterName, final JsonElement element,
            final Set<String> modifiedParameters) {
        BigDecimal value = null;
        if (element.isJsonObject()) {
            final JsonObject object = element.getAsJsonObject();
            final Locale locale = Locale.forLanguageTag("en");
            value = extractBigDecimalName(parameterName, object, locale, modifiedParameters);
        }
        return value;
    }
